...
//class Key and T must have a default constructor.
...
class BPT {
    ...
    bool FindAndPrint(int addr, const string_t &key) {
        ...
        if (!nodeType) {    //body node
            ...
            int pos = 0;
            for (int i = tmp.n - 1; i >= 0; --i) {
                if (tmp.key[i].key < key) {
                    pos = i + 1;
                    break;
                }
            }
            return FindAndPrint(tmp.addr[pos], key);
        } else {    //leaf node
            leafNode tmp;
            read(tmp);
...
public:
    ...
    void findAndPrint(const string_t &key) {
        ...
        bool result = FindAndPrint(root, key);
